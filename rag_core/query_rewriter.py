...
class QueryRewriter:
    ...
    def _comprehensive_rewrite(self, query: str, analysis: Dict) -> List[str]:
        ...
        # 2. 查询扩展
        rewritten.extend(self._query_expansion(query, analysis))
        # 3. 句式重构
        rewritten.extend(self._sentence_restructuring(query))
        # 4. 领域特定重写
        rewritten.extend(self._domain_specific_rewrite(query, analysis))
        # 5. 多角度提问
        rewritten.extend(self._multi_perspective_questions(query, analysis))
    ...
    def _query_expansion(self, query: str, analysis: Dict) -> List[str]:
        """查询扩展"""
        expanded = []
        ...
        # 添加领域相关扩展
        domain_expansions = self._get_domain_expansions(query, analysis)
        expanded.extend(domain_expansions)

        return expanded

    def _sentence_restructuring(self, query: str) -> List[str]:
        """句式重构"""
        restructured = []
    ...
    def _domain_specific_rewrite(self, query: str, analysis: Dict) -> List[str]:
        """领域特定重写"""
        domain_rewrites = []
    ...
    def _multi_perspective_questions(self, query: str, analysis: Dict) -> List[str]:
        """多角度提问"""
        perspectives = []
        key_terms = analysis.get("key_terms", [])
    ...
    def _analyze_query(self, query: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "contains_question_words": self._contains_question_words(query),
            "query_type": self._classify_query_type(query),
            "complexity_level": self._assess_complexity(query),
            "key_terms": self._extract_key_terms(query),
            "domain": self._identify_domain(query),
            "potential_issues": self._identify_potential_issues(query)
        }
        return analysis
    ...
    def _get_domain_expansions(self, query: str, analysis: Dict) -> List[str]:
        """获取领域相关的扩展"""
        expansions = []
    ...
    def _contains_question_words(self, query: str) -> bool:
        """检查是否包含疑问词"""
    ...
    def _identify_potential_issues(self, query: str) -> List[str]:
        """识别潜在问题"""
        issues = []
