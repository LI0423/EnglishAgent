...
# 记录原始 agent_core.agent，测试结束后必须还原。
# 否则本模块注入的伪模块会残留在 sys.modules 中，污染后续被收集的测试模块
# （例如 test_week2_backend_progress.py 需要真实的 deep_search_agent）。
_ORIGINAL_AGENT_MODULE = sys.modules.get("agent_core.agent")
...
try:
    ...
    # 还原 sys.modules，避免污染其它测试模块的导入
    if _ORIGINAL_AGENT_MODULE is not None:
        sys.modules["agent_core.agent"] = _ORIGINAL_AGENT_MODULE
    else:
        sys.modules.pop("agent_core.agent", None)
